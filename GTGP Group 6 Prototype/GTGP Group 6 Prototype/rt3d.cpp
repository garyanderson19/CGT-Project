...
void setAtt(const GLuint program, const attenuationStruct att) {
	// pass in material data to shader 
	int uniformIndex = glGetUniformLocation(program, "attConst");
	glUniform1f(uniformIndex, att.constant);
	uniformIndex = glGetUniformLocation(program, "attLinear");
	glUniform1f(uniformIndex, att.linear);
	uniformIndex = glGetUniformLocation(program, "attQuadratic");
	glUniform1f(uniformIndex, att.quadratic);
}
